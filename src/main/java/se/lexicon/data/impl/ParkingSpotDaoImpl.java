...
public class ParkingSpotDaoImpl implements ParkingSpotDao {
    ...
    public ParkingSpot create (ParkingSpot parkingSpot) {
        if(parkingSpot == null) throw new IllegalArgumentException("Parking Spot can not be null");
        if(find(parkingSpot.getSpotNumber()).isPresent()) throw new IllegalArgumentException("Parking Spot already exists");
    ...
    public Optional<ParkingSpot> find (int spotNumber) {
        if (spotNumber < 1) throw new IllegalArgumentException ("Spot number must be greater than 1");;
    ...
    public List<ParkingSpot> findByAreaCode (int areaCode) {
        if(areaCode < 1) throw new IllegalArgumentException ("Area code must be greater than 1");
